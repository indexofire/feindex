...
class SignupForm(forms.Form):
    ...
    def save(self):
        ...
        new_user = AccountSignup.objects.create_user(username, email, password,
            not ACCOUNT_ACTIVATION_REQUIRED, ACCOUNT_ACTIVATION_REQUIRED)
        return new_user
...
class AuthenticationForm(forms.Form):
    ...
    remember_me = forms.BooleanField(
        widget=forms.CheckboxInput(attrs=attrs_dict),
        required=False,
        label=_(u'Remember me for %(days)s') % {'days': _(ACCOUNT_REMEMBER_ME_DAYS[0])},
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if ACCOUNT_SIGNIN_FIELD == 'email':
            self.fields['identification'] = identification_field_factory(
                _(u"Email"), _(u"Please supply your email."))
        ...
        if identification and password:
            ...
            if user is None:
                raise forms.ValidationError(_(u"Please enter a correct \
                    username or email and password. Note that both fields are \
                    case-sensitive."))
        return self.cleaned_data
