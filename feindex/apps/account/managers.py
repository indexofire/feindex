...
class AccountManager(UserManager):
    ...
    def create_user(self, username, email, password, active=False,
        ...
        account_profile = self.create_userena_profile(new_user)
        ...
        if send_email:
            account_profile.send_activation_email()
    ...
    def confirm_email(self, username, confirmation_key):
        ...
        if SHA1_RE.search(confirmation_key):
            try:
                ...
                account.email_confirmation_key = ''
                account.save(using=self._db)
                user.save(using=self._db)
                account_signals.confirmation_complete.send(sender=None,
                    user=user)
                return user
        return False
    ...
    def delete_expired_users(self):
        ...
        for user in User.objects.filter(is_staff=False, is_active=False):
            if user.account_signup.activation_key_expired():
...
class AccountBaseProfileManager(models.Manager):
    """
