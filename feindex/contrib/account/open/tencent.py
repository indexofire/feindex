...
class QQWeiboAuthClient(oauth.OAuthClient):
    ...
    def fetch_request_token(self, callback_url=None):
        ...
        return oauth.OAuthToken.from_string(urlopen(Request(
            oauth_request.to_url())).read())

    def authorize_token_url(self,token):
        oauth_request = oauth.OAuthRequest.from_consumer_and_token(
            self.consumer, token=token, http_url=self.authorize_url,
            callback=self.callback_url)
    ...
    def get_authed_url(self,url,method,parameters):
        oauth_request=oauth.OAuthRequest.from_consumer_and_token(
            self.consumer, token=self.access_token, http_method=method,
