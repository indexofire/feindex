...
from .backends import AccountAuthenticationBackend
...
@secure_required
def account_signup(request, signup_form=SignupForm, success_url=None,
    template_name='account/signup_form.html', extra_context=None,):
    """
...
@secure_required
def activate(request, username, activation_key, success_url=None,
    template_name='account/activate_fail.html', extra_context=None):
    """
    ...
    """
    user = AccountSignup.objects.activate_user(username, activation_key)
    if user:
        # Sign the user in.
        ...
        else: redirect_to = reverse('account-profile-detail',
            kwargs={'username': user.username})
        return redirect(redirect_to)
    else:
        if not extra_context: extra_context = dict()
...
@secure_required
def email_confirm(request, username, confirmation_key, success_url=None,
    template_name='account/email_confirm_fail.html', extra_context=None):
    """
    ...
    if user:
        if success_url: redirect_to = success_url
        else: redirect_to = reverse('account-email-confirm-complete',
            kwargs={'username': user.username})
        return redirect(redirect_to)
    else:
        if not extra_context: extra_context = dict()
...
@secure_required
def account_signin(request, auth_form=AuthenticationForm,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user.is_active:
                login(request, user)
                if remember_me:
                    request.session.set_expiry(ACCOUNT_REMEMBER_ME_DAYS[1] * 86400)
                else:
...
def email_change(request, username, form=ChangeEmailForm,
    template_name='account/email_form.html', success_url=None,
    extra_context=None):
    """
    ...
    **Todo**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if success_url:
                redirect_to = success_url
            else: redirect_to = reverse('account-email-change-complete',
                kwargs={'username': user.username})
            return redirect(redirect_to)
...
@secure_required
@permission_required_or_403('change_user', (User, 'username', 'username'))
def password_change(request, username, success_url=None,
    template_name='account/password_form.html', pass_form=PasswordChangeForm,
    extra_context=None):
    """
    ...
    **Context**
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if success_url: redirect_to = success_url
            else: redirect_to = reverse('account-password-change-complete',
                kwargs={'username': user.username})
            return redirect(redirect_to)
...
def profile_edit(request, username, edit_profile_form=EditProfileForm,
    template_name='account/profile_form.html', success_url=None,
    extra_context=None):
    """
    Edit profile.
...
def profile_detail(request, username,
    template_name='account/profile_detail.html', extra_context=None):
    """
...
def profile_list(request, page=1, template_name='account/profile_list.html',
    paginate_by=50, extra_context=None, **kwargs):
    """
    Returns a list of all profiles which request user could see.

    It's possible to disable this by changing ``ACCOUNT_DISABLE_PROFILE_LIST``
    ...
    **Context**
    ...
    if ACCOUNT_DISABLE_PROFILE_LIST and not request.user.is_staff:
        raise Http404
    ...
    return list_detail.object_list(request, queryset=queryset,
        paginate_by=paginate_by, page=page, template_name=template_name,
        extra_context=extra_context, template_object_name='profile', **kwargs)
